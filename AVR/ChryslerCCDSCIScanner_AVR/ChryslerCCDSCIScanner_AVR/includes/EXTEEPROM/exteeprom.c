/*-----------------------------------------------------------------------------*
 * ext_eeprom.cpp - Arduino library to support external I2C EEPROMs.           *
 *                                                                             *
...
// - eepromAddr is the EEPROM's I2C address, default is 0x50 which is common.
void exteeprom_init(uint16_t deviceCapacity, uint8_t nDevice, uint16_t pageSize, uint8_t eepromAddr)
{
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
uint8_t exteeprom_write(uint32_t addr, uint8_t value)
{
    return exteeprom_write_bytes(addr, &value, 1);
}
...
int16_t exteeprom_read(uint32_t addr)
{
    uint8_t data;
    int16_t ret;
    
    ret = exteeprom_read_bytes(addr, &data, 1);
